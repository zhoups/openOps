...
def asset_add(request):
    if request.method == 'POST':
        ...
        try:
            host = Asset.objects.get(hostname=hostname)
        except:
            host = None
        if host:
            return JsonResponse({"msg":"主机已存在"})

        ip = request.POST.get('ip')
        port = request.POST.get('port')
        print(ip,port,host)
        # admin_user = random.choice(AdminUser.objects.all())
        # cluster = random.choice(Cluster.objects.all())
        # print(type(admin_user),cluster)
        asset = Asset(hostname=hostname, ip=ip, port=port, is_active=True,comment='')
        # asset.cpu_model = request.POST.get('cpu_model','1')
        asset.cpu_count = request.POST.get('cpu_count',2)
        asset.cpu_cores = request.POST.get('cpu_cores',3)
        asset.memory = request.POST.get('memory','4')
        asset.disk_total = request.POST.get('disk_total','5')
        asset.disk_info = request.POST.get('disk_info','6')
        asset.vendor = request.POST.get('vendor','7')
        asset.model = request.POST.get('model','8')
        asset.sn = request.POST.get('sn','9')
        asset.platform  = request.POST.get('platform','11')
        asset.os = request.POST.get('os','22')
        # asset.os_version = request.POST.get('os_version','33')
        # asset.os_arch = request.POST.get('os_arch','44')
        asset.hostname_raw = request.POST.get('hostname_raw','55')
        asset.created_by = request.POST.get('created_by','66')
...
class AssetUpdateView(AdminUserRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    success_url = reverse_lazy('assets:asset-list')#跳到这里
